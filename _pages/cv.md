---
layout: archive
title: "Curriculum Vitae"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

📄 [Download](/files/resume.pdf)


## EDUCATION

### Delft University of Technology — MSc. Marine Technology  
*Delft, Netherlands | Feb. 2024 – May 2026*

- *Coursework*: Marine Engineering, Ship Hydromechanics, Fluid Dynamics, Nuclear Reactor Physics
- *Thesis research*: Developed and applied an experimental setup and methodology for testing sails in wind tunnel inflow conditions representative of those encountered by wind assisted ships at sea
- Completed the program with an average grade of 8.2 with a 9.0 for the graduation thesis

### Delft University of Technology — BSc. Aerospace Engineering  
*Delft, Netherlands | Sep. 2017 – Feb. 2024*

- Completed the program with an average grade of 7.2
- Completed the Minor Education for which a physics teaching qualification (tweedegraadse lesbevoegdheid natuurkunde) was obtained
- Completed the Minor Computer Science with an average grade of 8.2

## WORK EXPERIENCE

### FlexDelft — Researcher  
*Delft, Netherlands | Jun 2026 – Jul 2026*

- Developing and testing new systems in the wind tunnel lab
- Implementing the methodology developed during the graduation thesis

### Vechtstede College — Teaching Assistant  
*Weesp, Netherlands | 2020–2021, 2023*

### Johan de Witt Scholengroep — Physics Teacher (Internship)  
*The Hague, Netherlands | 2022 – 2023*

## SKILLS

- **Programming:** Python (NumPy, SciPy, Matplotlib, Pandas), Numerical modeling (Fluid dynamics, Rigid body dynamics, Monte-Carlo), Data processing and visualization
- **Engineering Design:** Concept development, Design for manufacturing, CAD (Onshape, CATIA, CadQuery)
- **Hands-on:** General tool usage, Workshop fabrication, Machining (Turning, Milling), Experimental fluid dynamics
- **Tools:** MATLAB, Simulink, PowerPoint, LaTeX  

## VOLUNTEERING

### Stichting Vakantiespelen Dijkgatsbos — Camp Counselor  
*Wieringerwerf | Aug. 2023*

### Scouting Flevo Zee — Scout Leader  
*Huizen | 2016 – 2023*

## HOBBIES

Programming, Sailing, Making things (see portfolio)
